...
export default function AIAssistant() {
  ...
  const renderMessageContent = (text) => {
    return text.split("\n").map((line, idx) => {
      ...
      content = content.replace(
        /\$\$(.*?)\$\$/g,
        '<code style="background:var(--bg-tertiary); padding:4px 8px; border-radius:4px; display:block; margin:8px 0; font-family:monospace;">$1</code>',
      );
      content = content.replace(
        /\$(.*?)\$/g,
        '<code style="background:var(--bg-tertiary); padding:2px 6px; border-radius:4px; font-family:monospace;">$1</code>',
      );
    ...
    try {
      if (settings.anthropicKey) {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errData.error?.message || `API error code ${response.status}`,
          );
        }
      ...
      addToast(
        lang === "ar"
          ? "فشلت مكالمة المساعد الذكي. يرجى التحقق من المفتاح."
          : "Claude API call failed. Check your API key.",
        "error",
      );

      const aiResponse =
        `*API Request Error: ${error.message}*\n\n` +
        generateLocalAIResponse(userText);
      setMessages((prev) => [
        ...prev,
        {
          id: nextId(),
  ...
  return (
    <div className="chat-container page-enter">
      {/* Page Hero */}
      <div className="page-hero" style={{ '--hero-glow': 'rgba(0,242,254,0.15)', '--hero-gradient': 'linear-gradient(135deg, #00f2fe, #4facfe)' }}>
        <div className="page-hero-content">
          <div className="page-hero-icon-wrapper">
            <Sparkles size={32} color="#fff" />
          </div>
          <h1 className="page-hero-title">{t("ai-assistant")}</h1>
        ...
        {!settings.anthropicKey && (
          <div className="page-hero-actions">
            ...
            >
              <Key size={16} />{" "}
              {lang === "ar"
                ? "أضف رمز Claude في الإعدادات لإجابات حية"
                : "Add Claude key in Settings for live answers"}
            </div>
          </div>
        )}
      </div>
      ...
      >
        <div className="chat-messages">
          ...
          {isLoading && (
            ...
            >
              ...
              <span>
                {lang === "ar"
                  ? "المعلم الذكي يفكر حالياً..."
                  : "Di0 Tutor is thinking..."}
              </span>
            </div>
          )}
          <div ref={chatEndRef} />
        ...
        >
          <input
            ...
            placeholder={
              lang === "ar"
                ? "اسأل المعلم الذكي عن أي موضوع دراسي..."
